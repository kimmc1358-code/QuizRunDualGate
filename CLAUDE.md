...
from the card, because those two plates do not grow when the card does. A
debug build ignores the lock entirely, so the actual refusal only shows in a
release export.
